...
public interface TestQuestionRepository extends JpaRepository<TestQuestion, Long> {
    ...
    @Query(value = "select question.question_text, count (question.id) from question join question_test_answers on question.id = question_test_answers.test_question_id join test_answer on question_test_answers.test_answers_id = test_answer.id where question.id = ?1 group by test_answer.answer_text", nativeQuery = true)
